...
def PolyFitLS(X,Y,n):
    ...
    plt.plot(X,arr_f,'bo:',label='Fitted Polynomial')
    plt.plot(X,Y,'ro',label='Orignal Values')
...
#z=MidInt(1,2,N,f1)
#z1=TzInt(1,2,N,f1)
#print('For N =',N,'the integral of f1 is',z,z1)
...
def MonteCarlo(f1,A,B,N):
    F=0
    
    RR=[]

    for i in range(0,N):
        r=toolsar.rnumbwAB(r,A,B)
        RR.append(r)
...
def RKXY(d2ydx2, dydx, x0, y0, z0, xf, st, yes,xlabel,ylabel):
...
def ShootXY(d2ydx2, dydx, x0, xf, y0, yf, z0, dx, tol, yes):
    ...
    while abs(yff-yf) >= tol:
        ...
        plt.plot(x0_1,y0_1,':',label='guess plot')
        ...
        plt.plot(x0_11,y0_11,':',label='guess plot')
    ...
    if str(yes)=='yes':
        plt.plot(x0_1,y0_1,'r:',label="Final Plot")
        plt.legend()
        plt.title('Plot of the solution')
        plt.show()
        print('Line here is not a fitting of the polynomial. Has been added to aid the eye to track the points.')
...
#Vt,LX,V0 = ExpPDE(ux0, 2, 4, 20, 5000, 50)

#plt.plot(LX,V0,'r-',label='Time Step = 0')
...
#arr_z,arr_x = PredictorCorrector(f,0,.1,10,0)
#arr_y,arr_x = ForwardEuler(f,0,.1,10,0)

def ForwardEuler(f,x0,dx,xf,y0):
    arr_x=[]
    arr_y=[]
    xt=x0
    y=y0
...
def PredictorCorrector(f,x0,dx,xf,y0):
    arr_y=[]
    arr_x=[]
    arr_z=[]
    xt=x0
    ...
    while xt<=xf:
        ...
        if len(arr_z)==0:
            arr_z.append(y+((k2-k1)/2))
            #print(arr_z[len(arr_z)-1],y,k1,k2)
        else:
            arr_z.append(arr_z[len(arr_z)-1] +((k2+k1)/2))
            #print(arr_z[len(arr_z)-1],k1,k2)
        
        xt += dx
        
    plt.plot(arr_x,arr_z,'ro-')
    plt.title('Plot of the solution')
    plt.show()
    return arr_z,arr_x
